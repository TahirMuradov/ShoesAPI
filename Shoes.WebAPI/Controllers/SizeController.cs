...
namespace Shoes.WebAPI.Controllers
{
    ...
    public class SizeController : ControllerBase
    {
       private readonly ISizeService _sizeService;
        ...
        [HttpDelete("[action]")]
        public IActionResult DeleteSize([FromBody] Guid Id)
        {
            var result = _sizeService.DeleteSize(Id);
            return result.IsSuccess ? Ok(result) : BadRequest();
        }
        [HttpGet("[action]")]
        public IActionResult GetSize([FromBody] Guid Id)
        {
            var result = _sizeService.GetSize(Id);
            return result.IsSuccess ? Ok(result) : BadRequest();
        }
        [HttpGet("[action]")]
        public async Task<IActionResult> GetAllSize([FromBody] int page)
        {

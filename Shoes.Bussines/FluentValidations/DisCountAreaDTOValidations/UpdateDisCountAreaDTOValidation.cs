...
namespace Shoes.Bussines.FluentValidations.DisCountAreaDTOValidations
{
    ...
    {
        public UpdateDisCountAreaDTOValidation(string culture)
        {
            string[] SupportedLaunguages = ConfigurationHelper.config.GetSection("SupportedLanguage:Launguages").Get<string[]>();
            RuleFor(dto => dto.Id)
             .NotEmpty()
                .WithMessage(ValidatorOptions.Global.LanguageManager.GetString("IdIsRequired", new CultureInfo(culture)))
            .NotNull()
                .WithMessage(ValidatorOptions.Global.LanguageManager.GetString("IdIsRequired", new CultureInfo(culture)));
